...
- Optional CTA: target promotion site/domain and up to 2 anchor texts if provided. All first-party target-page links count together toward the two-link limit.
...
- Promotional links: if a target promotion site or URL is provided, links to that target site and all of its first-party pages appear no more than twice in total, once on the first natural mention and once on the final natural mention or CTA.
...
## Promotional Link Rules

When the user provides a target promotion site, product page, referral URL, landing page, or CTA URL:

- Treat the supplied target domain and all of its first-party pages as one promotion destination. For example, links to a target homepage, product page, model page, feature page, and pricing page count together.
- Use at most 2 Markdown links to that promotion destination in the article body.
- Add the first link on the first natural mention of the target product, website, tool, or offer.
- Add the second link on the final natural mention, usually in the conclusion or final CTA.
- Use natural anchor text. If the user provides 2 anchor texts, use the first anchor on the first mention and the second anchor on the final mention.
- Do not link every repeated mention of the promoted target.
- If the article only has one natural mention, use only one target link rather than forcing a second link.
- This limit applies only to the target promotion destination; third-party source citations, official documentation, and non-promotional reference links may still be used when needed for factual support.
...
- Avoid Markdown tables by default because many publishing editors do not preserve them well. Use bullet lists, short subsections, comparison cards, or definition-style blocks instead unless the user explicitly asks for a table.
...
- Every final article should include image support by default. Generate and insert at least one `hero-16x9.png` when image generation is available. For tutorials, comparisons, reviews, prompt guides, and long explainers, also prepare 1 to 2 section images when they help comprehension.
- If image generation is not used or cannot save usable files, create `writer/output/<article-slug>/image-plan.md` with filenames, placements, alt text, and ready-to-use 16:9 image prompts. Do not silently omit image guidance.
...
- The published article does not rely on Markdown tables unless the user explicitly requested them.
- Target promotion links, when provided, appear no more than twice: first natural mention and final natural mention or CTA.
